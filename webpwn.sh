...
phase_recon() {
    ...
    # Passive: bbot — uses -f subdomain-enum flag for full module coverage
    if require_tool bbot; then
        log_info "Running bbot (subdomain-enum flag, max 10m)..."
        local _bbot_out="$OUTDIR/recon/bbot"
        local _bbot_log="$_bbot_out/bbot_run.log"
        mkdir -p "$_bbot_out"

        # Pre-install all bbot module dependencies to prevent silent failures
        log_info "  Ensuring bbot module dependencies are installed..."
        bbot --install-all-deps -y 2>&1 | tail -5 | while IFS= read -r line; do log_dim "  bbot-deps: $line"; done || true

        # Use -f subdomain-enum (51 modules) instead of manually listing modules.
        # Use -om subdomains to get a clean subdomain-only output file.
        # --force-deps: retry any failed deps at runtime.
        # --ignore-failed-deps: don't abort if optional modules can't install deps.
        # Removed --silent: it suppresses stdout event lines we were trying to parse.
        # Instead, capture all output to log file and extract from bbot's own output files.
        local _bbot_rc=0
        timeout 600 bbot -t "$TARGET" \
            -f subdomain-enum \
            -rf passive \
            --allow-deadly \
            --force-deps \
            ...
            --force \
            -y \
            -om subdomains csv txt json 2>&1 | tee "$_bbot_log" | \
            grep -oP '(?<=\[DNS_NAME\])\s+\S+' | \
            awk '{print $1}' | \
            grep -iP "(^|\.)${TARGET//./\\.}$" | \
            anew_append "$ALL_SUBS" >/dev/null || _bbot_rc=$?

        # Primary extraction: bbot's subdomains output module writes a clean file
        local _bbot_subs_file
        _bbot_subs_file=$(find "$_bbot_out" -name "subdomains.txt" -type f 2>/dev/null | head -1)
        if [[ -n "$_bbot_subs_file" && -s "$_bbot_subs_file" ]]; then
            local _n_subs
            _n_subs=$(grep -icP "(^|\.)${TARGET//./\\.}$" "$_bbot_subs_file" 2>/dev/null || echo 0)
            grep -iP "(^|\.)${TARGET//./\\.}$" "$_bbot_subs_file" 2>/dev/null | \
                anew_append "$ALL_SUBS" >/dev/null || true
            log_ok "  bbot subdomains.txt: ${_n_subs} in-scope entries"
        fi

        # Fallback: parse output.txt for DNS_NAME events
        local _bbot_output_txt
        _bbot_output_txt=$(find "$_bbot_out" -name "output.txt" -type f 2>/dev/null | head -1)
        if [[ -n "$_bbot_output_txt" && -s "$_bbot_output_txt" ]]; then
            grep '\[DNS_NAME\]' "$_bbot_output_txt" 2>/dev/null | \
                awk '{print $2}' | \
                grep -iP "(^|\.)${TARGET//./\\.}$" | \
                anew_append "$ALL_SUBS" >/dev/null || true
        fi

        # Fallback: parse output.csv for DNS_NAME rows
        local _bbot_output_csv
        _bbot_output_csv=$(find "$_bbot_out" -name "output.csv" -type f 2>/dev/null | head -1)
        if [[ -n "$_bbot_output_csv" && -s "$_bbot_output_csv" ]]; then
            awk -F',' '$1=="DNS_NAME"{print $2}' "$_bbot_output_csv" 2>/dev/null | \
                tr -d '"' | \
                grep -iP "(^|\.)${TARGET//./\\.}$" | \
                anew_append "$ALL_SUBS" >/dev/null || true
        fi
        ...
        local _bbot_total
        _bbot_total=$(wc -l < "$ALL_SUBS" 2>/dev/null || echo 0)
        if [[ "$_bbot_rc" -ne 0 && "$_bbot_rc" -ne 124 ]]; then
            log_warn "  bbot exited with code ${_bbot_rc} — check log: ${_bbot_log}"
        elif [[ "$_bbot_rc" -eq 124 ]]; then
            log_warn "  bbot timed out (10m limit) — partial results collected"
        fi
        log_ok "  bbot: total subs in ALL_SUBS after bbot: ${_bbot_total}"
    fi
